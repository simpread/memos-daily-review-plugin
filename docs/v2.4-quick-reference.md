# v2.4 快速参考指南

## 🚀 新功能速览

### 1. localStorage 配额监控

**自动监控**（无需手动操作）:
- ✅ 每 10 分钟自动输出存储报告
- ✅ 历史清理后自动输出
- ✅ 配额超限前自动输出

**手动查看**（浏览器控制台）:
```javascript
// 查看存储统计
storageUtils.calculateStorageStats()

// 输出格式化报告
storageUtils.logStorageReport()
```

**报告示例**:
```
[DailyReview] Storage Usage Report
==================================================
  pool                     45.23 KB ( 52.1%)
  history                  28.67 KB ( 33.0%)
  cache                    10.45 KB ( 12.0%)
  settings                  1.89 KB (  2.2%)
  capabilities              0.45 KB (  0.5%)
  batch                     0.12 KB (  0.1%)
  ----------------------------------------------
  Total                    86.81 KB
```

---

### 2. markdownToHtml 重构

**用户影响**: 无（100% 向后兼容）

**开发者收益**:
- ✅ 代码复杂度降低 52%
- ✅ 更易于维护和扩展
- ✅ 更好的测试覆盖

**测试新模块**（浏览器控制台）:
```javascript
// 测试缩进深度计算
const calc = utils.IndentDepthCalculator.create()
calc.getDepth(0)  // 0
calc.getDepth(2)  // 1
calc.getDepth(4)  // 2

// 测试 Markdown 渲染
const html = utils.markdownToHtml('# Test\n- Item 1\n  - Nested')
console.log(html)
```

---

### 3. CHANGELOG.md

**位置**: 项目根目录

**内容**: 完整的版本历史（v1.0.0 到 v2.4.0）

**链接**:
- [CHANGELOG.md](../CHANGELOG.md)
- [README.md](../README.md) 中的文档部分
- [中文 README](../docs/zh-CN/README.zh-CN.md) 中的文档部分

---

## 📋 常用命令

### 测试命令

```bash
# 语法检查
node --check memos-daily-review-plugin.js

# v2.4 功能测试
node --test tests/v2.4-improvements.test.js

# 回归测试
node --test tests/algorithm.test.js

# 完整验证
bash scripts/verify-v2.4.sh
```

### 部署命令

```bash
# 备份当前版本
cp memos-daily-review-plugin.js memos-daily-review-plugin.v2.3.js.bak

# 验证新版本
bash verify-v2.4.sh

# 回滚（如需要）
cp memos-daily-review-plugin.v2.3.js.bak memos-daily-review-plugin.js
```

---

## 🔍 调试技巧

### 查看存储使用

```javascript
// 方法 1: 完整报告
storageUtils.logStorageReport()

// 方法 2: 原始数据
const stats = storageUtils.calculateStorageStats()
console.log(stats)
```

### 测试 Markdown 渲染

```javascript
// 简单测试
utils.markdownToHtml('# Heading\n- Item 1\n- Item 2')

// 嵌套列表测试
utils.markdownToHtml(`
- Level 1
  - Level 2
    - Level 3
  - Back to 2
- Back to 1
`)

// 混合列表测试
utils.markdownToHtml(`
- Unordered
  1. Ordered nested
  2. Another ordered
- Back to unordered
`)
```

### 检查缩进深度

```javascript
const calc = utils.IndentDepthCalculator.create()

// 测试基础缩进
console.log(calc.getDepth(0))  // 0
console.log(calc.getDepth(2))  // 1
console.log(calc.getDepth(4))  // 2

// 测试反缩进
console.log(calc.getDepth(2))  // 1
console.log(calc.getDepth(0))  // 0

// 重置
calc.reset()
console.log(calc.stack)  // []
```

---

## 📊 性能监控

### 监控指标

| 指标 | 预期值 | 检查方法 |
|------|--------|----------|
| Deck 生成 | < 100ms | 浏览器 Performance 面板 |
| Markdown 渲染 | < 50ms | 浏览器 Performance 面板 |
| 存储监控 | 可忽略 | 控制台输出频率 |
| 内存使用 | 稳定 | 浏览器 Memory 面板 |

### 性能测试

```javascript
// Deck 生成性能
performance.mark('deck-start')
await controller.loadDeck()
performance.mark('deck-end')
performance.measure('deck', 'deck-start', 'deck-end')
console.log(performance.getEntriesByName('deck'))

// Markdown 渲染性能
performance.mark('md-start')
utils.markdownToHtml(longText)
performance.mark('md-end')
performance.measure('md', 'md-start', 'md-end')
console.log(performance.getEntriesByName('md'))
```

---

## 🐛 故障排查

### 问题 1: 存储报告未输出

**可能原因**:
- 未到 10 分钟间隔
- 控制台被清空

**解决方法**:
```javascript
// 手动触发
storageUtils.logStorageReport()
```

### 问题 2: Markdown 渲染异常

**可能原因**:
- 输入包含特殊字符
- 嵌套层级过深

**解决方法**:
```javascript
// 检查输入
console.log('Input:', text)

// 检查输出
const html = utils.markdownToHtml(text)
console.log('Output:', html)

// 检查错误
try {
  utils.markdownToHtml(text)
} catch (e) {
  console.error('Error:', e)
}
```

### 问题 3: 测试失败

**可能原因**:
- Node.js 版本过低
- 依赖缺失

**解决方法**:
```bash
# 检查 Node.js 版本（需要 >= 18）
node --version

# 重新运行测试
node --test tests/v2.4-improvements.test.js
```

---

## 📚 相关文档

### 核心文档

- [CHANGELOG.md](../CHANGELOG.md) - 版本历史
- [README.md](../README.md) - 用户文档
- [CONTRIBUTING.md](../CONTRIBUTING.md) - 开发指南
- [CLAUDE.md](../CLAUDE.md) - AI 开发参考

### v2.4 专属文档

- [v2.4-features.md](./v2.4-features.md) - 功能使用指南
- [v2.4-implementation-complete.md](../v2.4-implementation-complete.md) - 实施完成报告
- [v2.4-improvements.test.js](../tests/v2.4-improvements.test.js) - 单元测试
- [verify-v2.4.sh](../verify-v2.4.sh) - 验证脚本

---

## 🔗 快速链接

### GitHub

- **仓库**: https://github.com/Kenshin/memos-daily-review-plugin
- **Issues**: https://github.com/Kenshin/memos-daily-review-plugin/issues
- **Releases**: https://github.com/Kenshin/memos-daily-review-plugin/releases

### 版本比较

- [v2.4.0...v2.3.0](https://github.com/Kenshin/memos-daily-review-plugin/compare/v2.3.0...v2.4.0)
- [v2.3.0...v2.2.0](https://github.com/Kenshin/memos-daily-review-plugin/compare/v2.2.0...v2.3.0)
- [v2.2.0...v2.1.0](https://github.com/Kenshin/memos-daily-review-plugin/compare/v2.1.0...v2.2.0)

---

## ❓ 常见问题

### Q: v2.4 需要清理缓存吗？

A: 不需要。v2.4 完全向后兼容，无需清理缓存或修改配置。

### Q: 存储报告会影响性能吗？

A: 不会。报告仅每 10 分钟生成一次，CPU 和内存开销可忽略。

### Q: Markdown 渲染有变化吗？

A: 没有。重构仅改变内部实现，输出完全一致。

### Q: 如何禁用存储报告？

A: 存储报告仅输出到控制台，不影响用户。如需禁用，可注释掉 `storageMonitor.checkAndCleanup()` 中的 `logStorageReport()` 调用。

### Q: 支持哪些浏览器？

A: Chrome/Edge/Firefox/Safari 现代版本。IE11 部分支持（Blob API 降级处理）。

### Q: 如何回滚到 v2.3？

A:
```bash
cp memos-daily-review-plugin.v2.3.js.bak memos-daily-review-plugin.js
```

---

## 💡 最佳实践

### 开发建议

1. **使用验证脚本**: 每次修改后运行 `bash verify-v2.4.sh`
2. **查看存储报告**: 定期检查存储使用情况
3. **运行测试**: 修改代码后运行完整测试套件
4. **阅读 CHANGELOG**: 了解每个版本的变更

### 部署建议

1. **先备份**: 部署前备份当前版本
2. **先验证**: 使用验证脚本确认无问题
3. **监控反馈**: 部署后关注用户反馈和性能指标
4. **准备回滚**: 保留备份文件以便快速回滚

---

## 📞 获取帮助

### 报告问题

1. 检查 [CHANGELOG.md](../CHANGELOG.md) 确认不是已知问题
2. 搜索 [GitHub Issues](https://github.com/Kenshin/memos-daily-review-plugin/issues)
3. 创建新 Issue，包含：
   - 问题描述
   - 复现步骤
   - 浏览器版本
   - 控制台错误信息

### 贡献代码

1. 阅读 [CONTRIBUTING.md](../CONTRIBUTING.md)
2. Fork 仓库
3. 创建功能分支
4. 提交 Pull Request

---

**版本**: v2.4.0
**更新时间**: 2026-02-24
**维护者**: Memos Daily Review Plugin Team
